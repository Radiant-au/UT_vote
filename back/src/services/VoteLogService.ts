import { VoteCountDto, VoteCountsResponseDto, VoteLogRequest, VoteLogResponse } from "@dtos/VoteLogDto";
...
export class VoteLogService {
    ...
    static async insertVoteLog(voteLog: VoteLogRequest , codeId : number): Promise<VoteLogResponse> {
        const pinCode = await PinCodeRepository.findOneBy({id : codeId});
        const maleSeletion = await SelectionRepository.findOneBy({id : voteLog.maleId});
        const femaleSeletion = await SelectionRepository.findOneBy({id : voteLog.femaleId});
