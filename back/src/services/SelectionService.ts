...
export class SelectionService {
    ...
    static async createSelection(data: SelectionRequest, profileImage?: string): Promise<SelectionResponse> {
    ...
    static async updateSelection(id: number, data: SelectionRequest, profileImage?: string): Promise<SelectionResponse> {
        const selection = await SelectionRepository.findOneBy({ id });
        if (!selection) {
            throw new Error('Selection not found');
        }
